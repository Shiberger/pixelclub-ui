...
export function RewardCell({ reward, onClaim }: { reward: BattlepassReward; onClaim: () => void }) {
  ...
  return (
    <button
      ...
      className="group relative aspect-square w-full overflow-hidden rounded-[9px] border-[3px] transition-transform duration-150 enabled:hover:scale-[1.06] enabled:active:scale-95"
      style={{
    ...
    >
      ...
      <span className="absolute top-[3px] left-[3px] rounded-[4px] bg-black/80 px-[5px] text-[10px] leading-[14px] font-bold tabular-nums">
        {formatQty(reward.item.qty)}x
      </span>

      <span className="txt-stroke-xs absolute inset-x-0 bottom-0 line-clamp-2 bg-gradient-to-t from-black/90 to-transparent px-1 pt-2 pb-[1px] text-center text-[9px] leading-[10px] font-bold">
        {reward.item.name}
      </span>

      {claimed && (
        <span className="absolute inset-0 grid place-items-center bg-black/45 text-[26px] text-[var(--green-lt)] drop-shadow-[0_0_8px_rgba(126,211,33,.9)]">
          ✔
        </span>
      )}
      {locked && <span className="absolute top-1 right-1 text-[12px] opacity-60">🔒</span>}
    </button>
  );
}
